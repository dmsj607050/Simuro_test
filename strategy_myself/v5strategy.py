...
def strategy_penalty(field):
    ...
    if race_state_trigger == Team.Opponent:
        if GlobalVariable.tick - tickBeginPenalty <= 60 and football_now_x <= -70 and football_now_y <= 40:
            ...
            if futureBallx < -50:
                baseRobots[3].moveto(-60, futureBally + 5)  # 3号机器人x不超过80的情况下追球（前锋）
            else:
                baseRobots[3].shoot(futureBallx, futureBally)  # 3号机器人x不超过80的情况下追球（前锋）

            if futureBallx > 20:
                baseRobots[2].moveto(20, futureBally)  # 2号机器人x不超过-12的情况下追球（后卫）
            elif futureBallx < -30:
                baseRobots[2].moveto(-30, futureBally - 5)  # 2号机器人x不超过-12的情况下追球（后卫）
            else:
                baseRobots.moveto(futureBallx - 10, futureBally)
            ...
            if oppRobots[0].get_pos().x < -71.5:
                baseRobots[0].moveto_within_x_limits(-110, -100, futureBallx, futureBally)
            else:
                baseRobots[0].moveto_within_x_limits(-110, -80, futureBallx, futureBally)
        else:
...
def strategy_penalty0(field):
    ...
    for i in range(0, 5):
        baseRobots[i].set_wheel_velocity(0, 0)
    if GlobalVariable.tick - tickBeginPenalty <= 2:
        baseRobots[1].set_wheel_velocity(125, -125)
        baseRobots[2].set_wheel_velocity(100, 125)
        baseRobots[3].set_wheel_velocity(125, 100)
    elif GlobalVariable.tick - tickBeginPenalty <= 10:
        baseRobots[1].set_wheel_velocity(125, 60)
        baseRobots[3].set_wheel_velocity(80, 80)
    elif GlobalVariable.tick - tickBeginPenalty <= 25:
        baseRobots[3].set_wheel_velocity(85, 85)
    elif GlobalVariable.tick - tickBeginPenalty <= 50:
        baseRobots[3].set_wheel_velocity(50, 117)
    else:
        if GlobalVariable.tick - tickBeginPenalty == 66:
            if race_state_trigger == 0 and race_state == 2:
                penalty_select = (penalty_select + 1) % 3
        strategy_common(field)
...
def strategy_penalty1(field):
    ...
    if GlobalVariable.tick - tickBeginPenalty <= 2:
        ...
        baseRobots[1].set_wheel_velocity(60, 125)
        baseRobots[3].set_wheel_velocity(80, 80)
    elif GlobalVariable.tick - tickBeginPenalty <= 25:
        baseRobots[3].set_wheel_velocity(85, 85)
    elif GlobalVariable.tick - tickBeginPenalty <= 50:
        baseRobots[3].set_wheel_velocity(117, 50)
    else:
...
def strategy_penalty2(field):
    ...
    for i in range(0, 5):
        baseRobots[i].set_wheel_velocity(0, 0)
    if GlobalVariable.tick - tickBeginPenalty <= 10:
        baseRobots[1].set_wheel_velocity(70, 70)
    elif GlobalVariable.tick - tickBeginPenalty <= 60:
        baseRobots[1].set_wheel_velocity(0, 125)
    else:
...
def strategy_goalkick(field):
    ...
    global goalkick_select
    global race_state_trigger
    if race_state_trigger == Team.Self:
        if goalkick_select == 0:
            goalkick0(field)
        if goalkick_select == 1:
            goalkick1(field)
        if goalkick_select == 2:
            goalkick2(field)
    if race_state_trigger == Team.Opponent:
        strategy_common(field)
...
def goalkick0(field):
    ...
    elif GlobalVariable.tick - tickBeginGoalKick <= 21:
        baseRobots[3].set_wheel_velocity(120, 120)
    elif GlobalVariable.tick - tickBeginGoalKick <= 40:
        baseRobots[4].set_wheel_velocity(120, 120)
    else:
        if GlobalVariable.tick - tickBeginGoalKick == 80:
            if race_state_trigger == 0 and race_state == 1:
                goalkick_select = (goalkick_select + 1) % 3
        strategy_common(field)
...
def goalkick1(field):
    ...
    if GlobalVariable.tick - tickBeginGoalKick <= 15:
        ...
        if GlobalVariable.tick - tickBeginGoalKick == 79:
            if race_state_trigger == 0 and race_state == 1:
                goalkick_select = (goalkick_select + 1) % 3
        strategy_common(field)
...
def goalkick2(field):
    ...
    if GlobalVariable.tick - tickBeginGoalKick <= 50:
        ...
        if GlobalVariable.tick - tickBeginGoalKick == 78:
            if race_state_trigger == 0 and race_state == 1:
                goalkick_select = (goalkick_select + 1) % 3
        strategy_common(field)
...
def strategy_placeKick(field):
    ...
    if race_state_trigger == Team.Self:
        ...
        if GlobalVariable.tick - tickBeginPlace <= 10:
            baseRobots[1].set_wheel_velocity(102, 102)
